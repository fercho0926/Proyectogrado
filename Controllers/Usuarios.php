        ...
        for ($i=0; $i < count($data); $i++) { 
            if ($data[$i]['estado'] == 1) {
                if ($data[$i]['id'] != 1) {
                    ...
                    $data[$i]['acciones'] = '<div>
                    <button title="Agregar permisos" class="btn btn-dark" onclick="btnRolesUser(' . $data[$i]['id'] . ')"><i class="fa fa-universal-access" aria-hidden="true"></i></button>
                    <button title="Editar" class="btn btn-primary" type="button" onclick="btnEditarUser(' . $data[$i]['id'] . ');"><i class="fa fa-pencil-square-o"></i></button>
                    <button title="Eliminar" class="btn btn-danger" type="button" onclick="btnEliminarUser(' . $data[$i]['id'] . ');"><i class="fa fa-trash-o"></i></button>

                    <button title="Restablecer una contraseña" class="btn btn-secondary type="button"   onclick="modificarClave(' . $data[$i]['id'] . ');"><i class="fa fa-key" aria-hidden="true"></i></button>
        ...
        if (empty($usuario) || empty($nombre)) {
            ...
            if ($id == "") {
                if (!empty($clave) && !empty($confirmar)) {
                    if ($clave != $confirmar) {
                        $msg = array('msg' => 'La contraseña no coinciden', 'icono' => 'warning');
                    } else {
        ...
        if (!$perm && $id_user != 1) {
            echo '<div class="card">
                    <div class="card-body text-center">
                        <span class="badge badge-dang">No tienes acceso</span>
                    </div>
                </div>';
            exit;
        }
